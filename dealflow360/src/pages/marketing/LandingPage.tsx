...
export function LandingPage() {
  return (
    <div className="min-h-screen bg-white">
      <header className="border-b border-[#e8eaed] sticky top-0 bg-white z-40">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm text-[#6b7280]">
            <a href="#product" className="hover:text-[#1a1d21]">Product</a>
            <a href="#how" className="hover:text-[#1a1d21]">How it works</a>
            <a href="#features" className="hover:text-[#1a1d21]">Features</a>
            <a href="#solutions" className="hover:text-[#1a1d21]">Solutions</a>
          </nav>
          <div className="flex items-center gap-2">
      ...
      <section className="max-w-6xl mx-auto px-4 py-16 lg:py-24 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          <p className="text-sm font-medium text-[#1565C0] mb-3">Sales freedom. Business control.</p>
          <h1 className="text-3xl lg:text-4xl font-bold text-[#1a1d21] leading-tight">
        ...
        <div className="bg-[#f5f6f8] border border-[#e8eaed] rounded-lg p-4 shadow-sm">
          <div className="bg-white rounded-md border border-[#e8eaed] overflow-hidden">
            <div className="px-4 py-2 border-b border-[#e8eaed] flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-[#2e7d32]" />
              <span className="text-xs text-[#6b7280]">DealFlow360 — Live Dashboard</span>
            </div>
            ...
            <div className="px-4 pb-4">
              <p className="text-xs text-[#6b7280] mb-2">Recent activity</p>
              {['Risk calculated — DF-1042', 'Manager approved — DF-1042', 'Allocation complete — DF-1028'].map((a) => (
                <div key={a} className="text-xs py-1.5 border-t border-[#e8eaed] text-[#1a1d21]">{a}</div>
              ))}
            </div>
          </div>
        </div>
      ...
      <section id="features" className="bg-[#fafbfc] border-y border-[#e8eaed] py-16">
        ...
        <div className="flex flex-wrap justify-center items-center gap-2 text-sm">
          {['Quote', 'Risk Evaluation', 'Approval', 'Fulfillment', 'Billing', 'Payment'].map((step, i, arr) => (
            <span key={step} className="flex items-center gap-2">
              <span className="px-4 py-2 bg-white border border-[#e8eaed] rounded-md font-medium">{step}</span>
      ...
      <section className="bg-[#1565C0] text-white py-16 text-center">
      ...
      <footer className="border-t border-[#e8eaed] py-8">
        <div className="max-w-6xl mx-auto px-4 flex flex-wrap justify-between gap-4 text-sm text-[#6b7280]">
...
function MiniStat({ label, value, sub, danger, success }: { label: string; value: string; sub: string; danger?: boolean; success?: boolean }) {
  return (
    <div className="p-3 bg-[#fafbfc] rounded border border-[#e8eaed]">
      ...
      <p className={`text-xs mt-0.5 ${danger ? 'text-[#c62828]' : success ? 'text-[#2e7d32]' : 'text-[#6b7280]'}`}>{sub}</p>
    </div>
  )
}
...
function Feature({ icon: Icon, title, desc }: { icon: typeof Shield; title: string; desc: string }) {
  return (
    <div className="bg-white border border-[#e8eaed] rounded-lg p-5">
      <div className="w-9 h-9 rounded-md bg-[#e3f2fd] flex items-center justify-center mb-3">
